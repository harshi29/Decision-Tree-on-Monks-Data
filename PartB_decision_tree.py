# -*- coding: utf-8 -*-
"""
Created on Fri Sep 27 12:24:27 2019
...
if __name__ == '__main__':
    ...
    avg_trnErr = {}
    avg_testErr = {}
    output_dir = './Output/PartB/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    for dataset in range(1,4):
        # Load the training data
        M = np.genfromtxt('./monks_data/monks-' + str(dataset) + '.train', missing_values=0, skip_header=0, delimiter=',', dtype=int)
        ytrn = M[:, 0]
        Xtrn = M[:, 1:]

        # Load the test data
        M = np.genfromtxt('./monks_data/monks-' + str(dataset) + '.test', missing_values=0, skip_header=0, delimiter=',', dtype=int)
        ytst = M[:, 0]
        Xtst = M[:, 1:]
        ...
        render_dot_file(dot_str, output_dir + 'PartB_Tree_Monks' + str(dataset))
        ...
        for d in range(1, 11, 1):
            trees[d] = id3(Xtrn, ytrn, max_depth=d)
            y_pred = [predict_example(x, trees[d]) for x in Xtrn]
            trnErr[d] = compute_error(ytrn, y_pred)
            y_pred = [predict_example(x, trees[d]) for x in Xtst]
            testErr[d] = compute_error(ytst, y_pred)

        # Plot all the models
        plots_dir = output_dir + 'PartB_Plot_Monks' + str(dataset)
        plt.figure()
        ...
        plt.title("Monk Data-" + str(dataset))
        ...
        avg_training_error = np.mean(list(trnErr.values()))
        avg_test_error = np.mean(list(testErr.values()))
        avg_trnErr[dataset] = (avg_training_error)*100
        avg_testErr[dataset] = (avg_test_error)*100   
    
    for dataset, error_value in avg_testErr.items():
        print("*****")
        print("Avg Training Error on Monk Data-" + str(dataset) + ": ", avg_trnErr[dataset])
        print("Avg Test Error on Monk Data-" + str(dataset) + ": ", error_value)
